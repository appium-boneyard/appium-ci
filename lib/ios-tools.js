...
function killAll (processes = ['instruments', 'simulator']) {
  ...
  return B.reduce(seq, function callFunctions (_, fn) { return fn(); }, null);
}
...
function spawnAsUser (user, cmd, args = []) {
  ...
  ).spread(function spawnCmd (stdout) {
...
function spawnAsCurrentUser (cmd, args = []) {
  ...
  return utils.exec('whoami').spread(function spawnCmdWithUser (stdout) {
