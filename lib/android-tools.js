  ...
  killAll: async (processes) => {
    ...
    for(let p of processes) {
      if (process.platform.match(/win/)) {
        await utils.exec('powershell -Command \'Stop-Process Name "*' + p +'*"\'' + p).catch(() => {});
      } else {
        await utils.exec('sudo pkill -f ' + p).catch(() => {});
      }
    }
  }
};
...
export class Emulator {
  ...
  async waitTillReady() {
    ...
    await B.race([_waitForEmu(this.opts.initWait), procPromise])
      .finally(() => {
        // cancel outstanding promise so that it does not hang the node process
